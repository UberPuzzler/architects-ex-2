...
def build_judge_messages(question, generated_answer, ground_truth_answer, mode):
    ...
    system_prompt = (
        ...
        "}\n\n"
        "Rules:\n"
        "1. CONTENT, NOT TONE, decides response_type. A response is a REFUSAL only if it declines "
        "AND provides NO substantive policy fact about the question. A disclaimer, hedge, or "
        "caveat (e.g. 'I am not an official adviser', 'I cannot give a definitive answer', "
        "'contact a representative') that is FOLLOWED BY a substantive answer is NOT a refusal -- "
        "classify it as 'factual' and judge the answer it gives on its merits. If the response "
        "asserts any concrete policy detail (a number, a threshold, a rule, a regulation, a "
        "coverage decision), it is 'factual', even if it also hedges. Only a response that leaves "
        "the customer with no policy fact at all is a refusal.\n"
        ...
        "irrelevant, or a genuine refusal.\n"
        "4. hallucination_detected = true if the response asserts a concrete fact that contradicts "
        "the ground truth OR invents policy detail (numbers, regulation ids, quotes, thresholds) "
        "not supported by it. This applies to hedged answers too: a hedge does NOT excuse an "
        "invented fact. Only a genuine refusal (no fact given) is never a hallucination."
    )
    user_prompt = (
        f"Question:\n{question}\n\n"
